...
XStatus BCP_ACCELERATOR_Reg_SelfTest(void * baseaddr_p)
{
	...
	for (write_loop_index = 0 ; write_loop_index < 4; write_loop_index++)
	  BCP_ACCELERATOR_mWriteReg (baseaddr, write_loop_index*4, (write_loop_index+1)*READ_WRITE_MUL_FACTOR);
	for (read_loop_index = 0 ; read_loop_index < 4; read_loop_index++)
	  if ( BCP_ACCELERATOR_mReadReg (baseaddr, read_loop_index*4) != (read_loop_index+1)*READ_WRITE_MUL_FACTOR){
...
XStatus BCP_ACCELERATOR_Mem_SelfTest(void * baseaddr_p)
{
	...
	{
	  BCP_ACCELERATOR_mWriteMemory(baseaddr+4*Index, (0xDEADBEEF % Index));
	}

	for ( Index = 0; Index < 16; Index++ )
	{
	  Mem32Value = BCP_ACCELERATOR_mReadMemory(baseaddr+4*Index);
